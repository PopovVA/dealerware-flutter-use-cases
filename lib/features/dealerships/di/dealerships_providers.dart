...
final dealershipsListNotifierProvider = Provider<DealershipsListNotifier>((
  ref,
) {
  return DealershipsListNotifier(
    getAllDealerships: ref.read(getDealershipsProvider),
    deleteDealership: ref.read(deleteDealershipProvider),
  );
});

/// Dealerships List State Provider - watch this for UI updates
final dealershipsListStateProvider = Provider<DealershipsListState>((ref) {
  final notifier = ref.watch(_dealershipsListNotifierProvider);
  // This creates a dependency on the notifier
  // We need to manually trigger rebuild when state changes
...
final dealershipDetailNotifierProvider = Provider<DealershipDetailNotifier>((
  ref,
) {
  return DealershipDetailNotifier(
    getDealershipById: ref.read(getDealershipByIdProvider),
    createDealership: ref.read(createDealershipProvider),
    updateDealership: ref.read(updateDealershipProvider),
  );
});
